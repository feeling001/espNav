...
> **Note on target limit:** to stay within the BLE attribute/MTU size limit, only the `BLE_AIS_MAX_TARGETS` (currently **3**) closest non-stale targets are sent per notification, sorted by ascending distance. This may be fewer than the total number of targets available via the REST API. A target is excluded once its AIS data is older than the configurable **AIS target retention time** (default **5 minutes**, adjustable from 10 s to 1 hour via `GET`/`POST /api/ais/config`, or from the dashboard's Config → AIS tab). AIS transponders do not report continuously — see the [REST API AIS Configuration section](./API_Backend_Documentation.md#91-ais-configuration) for details.
